...
function _in_npm_dir
  ls package.json > /dev/null 2>&1
end

function _npm_package_name
  echo (cat package.json | jq -r .name)
end

function _npm_package_version
...
function fish_prompt
  ...
  if _in_npm_dir
    _print_in_color ""(_npm_package_name) green
    _print_in_color "∙"(_npm_package_version) green
  end
